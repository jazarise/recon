/* StreamIn.java
 * basic stream input and lexical scan services
 * abstract class, must be tailored to actual lexical rules
 * for a particular file type
 * 
 * @author Ed Barkmeyer, June, 2011, based on the earlier FileIn
 */
...
/** class StreamIn
 * performs basic character management on a Reader (character input source)
 *  in Java terms, it is a filter
 * the constructor takes/records the Reader, the file name, and the error output stream
 *   and initializes the character buffering
...
public class StreamIn 
{
	...
	protected Reader fd;			/* actual file */
	protected PrintStream syserr;	/* file to send I/O error messages to */
	
	protected int thisc;	/* current character from EDI stream */
	protected int holdc;	/* look ahead character from stream, if any */
	protected int lastc;	/* previous character from stream */
	 ...
	 * if it cannot open the file, the StreamIn object is created,
	...
	{
		return getLine(user, usersize, 0);
	}
	
	public int getLine(char[] user, int usersize, int start)
	{
		for (cursize = start; cursize < usersize; nextC()) {
			if (thisc == EOF) return cursize - start;
			user[cursize++] = (char)thisc;
			if (thisc == '\n') {
				nextC();
				return cursize - start;
			}
		}
		return cursize - start;
	}

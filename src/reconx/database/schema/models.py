...
class WorkflowRunModel(Base):
    ...
    plugin_results = relationship(
        "PluginResultModel", back_populates="workflow_run", cascade="all, delete-orphan"
    )
...
class AssetModel(Base):
    ...
    relationships_as_source = relationship(
        "RelationshipModel",
        foreign_keys="[RelationshipModel.source_asset_id]",
        back_populates="source_asset_rel",
        cascade="all, delete-orphan",
    )
    relationships_as_target = relationship(
        "RelationshipModel",
        foreign_keys="[RelationshipModel.target_asset_id]",
        back_populates="target_asset_rel",
        cascade="all, delete-orphan",
    )
...
class RelationshipModel(Base):
    ...
    source_asset_id = Column(
        String, ForeignKey("assets.id", ondelete="CASCADE"), index=True, nullable=False
    )

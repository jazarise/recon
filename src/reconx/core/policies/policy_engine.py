        ...
        if event_type == "drift.detected":
            ...
            if drift_data.get("type") == "NEW_ASSET":
                ...
                if asset_type == "SUBDOMAIN":
                    print(
                        f"[ASM POLICY] Policy matched: new subdomain -> launching web.probe on {target}"
                    )
                ...
                # IF new js_file -> discovery.javascript
                    ...
                    print(
                        f"[ASM POLICY] Policy matched: new JS file -> launching JS Extractor on {target}"
                    )
                ...
                # IF new api endpoint -> discovery.api
                    ...
                    print(
                        f"[ASM POLICY] Policy matched: new API -> launching API Prober on {target}"
                    )

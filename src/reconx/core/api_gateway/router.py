...
class GatewayRouter:
    def handle_request(self, token: str, endpoint: str, required_role: str, action: str, target: str):
        ...
        if not user_data:
            audit_logger.log("UNKNOWN", action, target, "FAILED", "Invalid or expired token")
        ...
        if not has_permission(role, required_role):
            audit_logger.log(user, action, target, "FAILED", f"Permission denied (requires {required_role})")

...
class StrategyOptimizer:
    ...
    def adjust_weight(self, capability: str, delta: float):
        if capability in self.capability_weights:
            ...
            self.capability_weights[capability] = max(1.0, self.capability_weights[capability] + delta)
            print(f"[STRATEGY] Adjusted {capability} weight to {self.capability_weights[capability]:.2f} (Delta: {delta:+.2f})")

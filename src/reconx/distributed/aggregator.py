...
class CentralAggregator:
    ...
    async def process_worker_data(self, data: dict):
        ...
        logger.info(
            f"[AGGREGATOR] Processed {len(results)} findings for {target}. Total Global Nodes: {len(self.global_nodes)}"
        )

    def export_graph(self):
        return {"nodes": list(self.global_nodes), "edges": list(self.global_edges)}
